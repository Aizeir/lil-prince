...
class Timer:
    ...
    def __repr__(self):
        return f"Timer({self.active}{f', {self.time()}/{self.duration})' if self.active else ""})"
    ...
    def set_callback(self, func): self.callback = func
...
class Transition(Timer):
    ...
    def set_callback(self, func, type='mid'): (self.callmid,self.callend)[type=='end'] = func
    def set_callmid(self, func): self.callmid = func
    def set_callend(self, func): self.callend = func
